...
//				System.out.println("check");
            ...
            else if (destroy[y][x] > now.destroy)
                continue;
			for(int i=0;i<4;i++) {
				int nx=x+dx[i];
				int ny=y+dy[i];
				...
				if(arr[ny][nx]==1&&now.destroy==1)continue;
				if(arr[ny][nx]==1) {
					q.add(new Node(ny,nx,now.destroy-1,now.count+1));
					destroy[ny][nx]=now.destroy-1;
				}
				else {
					q.add(new Node(ny,nx,now.destroy,now.count+1));
	...
	static class Node{
		...
		int destroy;
		int count;
		Node(int y,int x,int destroy, int c){
			X=x;
			Y=y;
